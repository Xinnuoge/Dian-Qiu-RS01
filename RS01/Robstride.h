...
/* --- C 语言调用的包装接口 (在 main.c 中调用) --- */
void RS01_Init(uint8_t can_id);
void RS01_Enable(void);
void RS01_Disable(void);
void RS01_SetHome(void);
void RS01_Control_Motion(float torque, float angle, float speed, float kp, float kd);
void RS01_Control_Speed(float speed, float limit_cur);
void RS01_Control_Position(float speed, float angle);
void RS01_Parse_Feedback(uint8_t *data, uint32_t id);
float RS01_Get_Angle(void);
float RS01_Get_Speed(void);
int  RS01_IsReady(void);
void RS01_Init_Multi(uint8_t start_id, uint8_t count);
void RS01_Select_Motor(uint8_t can_id);
void Motor_OpControl(uint8_t can_id, float torque, float angle, float speed, float kp, float kd);
#ifdef __cplusplus
 }
#endif
