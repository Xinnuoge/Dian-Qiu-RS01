 ...
 * @brief  FDCAN2 发送函数
 * @param  id     : CAN ID
 * @param  idType : FDCAN_STANDARD_ID 或 FDCAN_EXTENDED_ID
 * @param  pData  : 8字节数据指针
 * @return HAL_StatusTypeDef : 返回发送状态
 */
static HAL_StatusTypeDef FDCAN2_Send(uint32_t id, uint32_t idType, uint8_t *pData) 
{
    FDCAN_TxHeaderTypeDef TxHeader;
    HAL_StatusTypeDef status;
    uint32_t retry_count = 0;
    const uint32_t MAX_RETRIES = 100; // 根据实时性要求调整重试次数

    // 1. 配置发送报文头 (灵足电机固定使用经典 CAN 模式)
    TxHeader.Identifier = id;
    TxHeader.IdType = idType;
    TxHeader.TxFrameType = FDCAN_DATA_FRAME;
    TxHeader.DataLength = FDCAN_DLC_BYTES_8;
    TxHeader.ErrorStateIndicator = FDCAN_ESI_ACTIVE;
    TxHeader.BitRateSwitch = FDCAN_BRS_OFF;   // 灵足电机不支持速率切换
    TxHeader.FDFormat = FDCAN_CLASSIC_CAN;    // 强制经典 CAN 格式
    TxHeader.TxEventFifoControl = FDCAN_NO_TX_EVENTS;
    TxHeader.MessageMarker = 0;

    // 2. 检查 Tx FIFO 是否有空位，避免直接调用导致的阻塞或失败
    // H7 的 FDCAN 有专门的 FIFO 状态检查
    while (HAL_FDCAN_GetTxFifoFreeLevel(&hfdcan2) == 0) {
        retry_count++;
        if (retry_count > MAX_RETRIES) {
            return HAL_BUSY; // 如果队列一直满，返回忙，由上层逻辑决定是否丢弃
        }
    }

    // 3. 将消息添加到发送队列
    status = HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan2, &TxHeader, pData);
...
void RobStride_Motor::RobStride_Get_CAN_ID()
{
	...
	uint32_t ext_id = (uint32_t)Communication_Type_Get_ID << 24 | (uint32_t)Master_CAN_ID << 8 | CAN_ID;
	FDCAN2_Send(ext_id, FDCAN_EXTENDED_ID, txdata);
}
/*******************************************************************************
...
void RobStride_Motor::RobStride_Motor_move_control(float Torque, float Angle, float Speed, float Kp, float Kd)
{
    ...
    uint32_t ext_id = (uint32_t)Communication_Type_MotionControl << 24 | 
                      (uint32_t)float_to_uint(Motor_Set_All.set_Torque, T_MIN, T_MAX, 16) << 8 | 
                      CAN_ID;
...
void RobStride_Motor::RobStride_Motor_MIT_Enable()
{
	...
	txdata[6] = 0xFF;
	txdata[7] = 0xFC;
	FDCAN2_Send(CAN_ID, FDCAN_STANDARD_ID, txdata);
}
...
void RobStride_Motor::RobStride_Motor_MIT_Control(float Angle, float Speed, float Kp, float Kd, float Torque)
{
	...
	FDCAN2_Send(CAN_ID, FDCAN_STANDARD_ID, txdata); 	//发送CAN消息
}
...
/*******************************************************************************
* @功能     		: RobStride电机位置模式(PP插补位置模式控制) - 优化版
* @参数1        : 目标角速度(-30rad/s~30rad/s)
* @参数2        : 目标角度(-4π~4π)
* @概述  				: 去掉了 HAL_Delay，改为基于状态判断的非阻塞控制
*******************************************************************************/
void RobStride_Motor::RobStride_Motor_Pos_control(float Speed, float Angle)
{
    ...
    // 检查电机当前运行模式是否为 PP位置模式 (1)
    if (drw.run_mode.data != Pos_control_mode)
    {
        // 模式不对时，仅发送切换指令和参数设置，不发送位置目标
    ...
    {
        // 只有当电机反馈 drw.run_mode.data 确实已经是 1 时，才发送位置指令
        // 这样保证了指令的有效性，且不阻塞主循环
...
/*******************************************************************************
* @功能     		: RobStride电机位置模式(CSP位置模式控制) - 优化版
*******************************************************************************/
...
void RobStride_Motor::Disenable_Motor(uint8_t clear_error)
{
    ...
    else
    {
        ...
        txdata[0] = clear_error; // 是否清除错误标志位
...
void RobStride_Motor::Set_RobStride_Motor_parameter(uint16_t Index, float Value, char Value_mode)
{
    ...
    if (Value_mode == 'p') // 设置具体参数值 (float)
    {
        memcpy(&txdata[4], &Value, 4);
    }
    else if (Value_mode == 'j') // 设置控制模式 (uint8)
    {
...
void RobStride_Motor::Set_ZeroPos()
{
    ...
    txdata[0] = 1; // 灵足协议规定：该位置 1 表示设置当前位置为机械零位
    FDCAN2_Send(ext_id, FDCAN_EXTENDED_ID, txdata);
    Enable_Motor();
}
